...
from app.core.security_alerts import (
    log_unauthorized_role_change,
    log_admin_transfer_success,
...
# Allows only the site admin to continue.
def _require_site_admin(db: Session, current_user: User) -> None:
    if current_user.global_role != GlobalRole.admin:
        log_access_denied_and_check_repeated(db, current_user, "not the site admin (admin panel)")
        raise HTTPException(
...
# Enforce the site-admin permission before reading any security alerts.
    _require_site_admin(db, current_user)
